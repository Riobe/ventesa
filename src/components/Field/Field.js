...
function Field() {
  ...
  const characterSprings = useSprings(charData.characters.length, charData.characters.map(char => ({
  ...
  const currentCharacter = charData.currentTurnIndex !== undefined && charData.characters[charData.currentTurnIndex];
  const atInitiative = currentCharacter ? currentCharacter.initiative : 'Not Started' ;
    ...
    <div className="Field">
      ...
      <div className="Field-characters">
        {characterSprings.map(({ top }, i) => (
            <animated.div key={charData.characters[i].name} className="Field-character" style={{
                top,
